...
proto._processLine = function _processLine(line) {
  ...
  var ms    = matches[1];
  var self  = parseInt(matches[2]);
  var depth = matches[3].length;
  ...
  parent.hitCount += self;
}

proto._genSamples = function genSamples(node) {
  for (var j = 0; j < node.hitCount; j++) this._samples.push(node.id);
  node.children.map(genSamples, this);
}
